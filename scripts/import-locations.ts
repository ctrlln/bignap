...
interface LocationRow {
    ...
    nurserylevel: string; // "Level III", etc.
    nurserylevel_id: string; // Add this
...
function runImport() {
    ...
    const records = parseCSV(csvContent) as unknown as LocationRow[]; // Type assertion for our interface
    ...
    db.transaction(() => {
        for (const record of records) {
            ...
            if (CITY_COORDS[cityKey]) {
                ...
                // Fuzzy fallback or log missing
                // console.log(`Missing coords for city: ${cityKey}`);
            }
